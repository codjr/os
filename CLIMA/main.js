...
const tempoDecorrido=ms=>{const d=Math.floor((Date.now()-ms)/60000);if(d<1)return"há segundos";if(d<60)return`há ${d} min`;if(d<1440)return`há ${(d/60).toFixed(1)} h`;return`há ${(d/1440).toFixed(1)} dias`;};

/* Online/offline */
function atualizarStatus(){
  if(navigator.onLine){statusBox.textContent="🟢 Online + cache ativo";statusBox.className="online";}
  else{statusBox.textContent="🔴 Offline (usando cache)";statusBox.className="offline";}
}
window.addEventListener("online",atualizarStatus);
window.addEventListener("offline",atualizarStatus);

/* Tema */
function aplicarTema(t){document.documentElement.dataset.tema=t;temaBtn.textContent=t==="dark"?"☀️":"🌙";localStorage.setItem(LS_THEME,t);}
...
function pushRecent(c){
  ...
  r.unshift(c);saveRecent(r);renderRecentChips(r);
}
function renderRecentChips(l){
  recentChips.innerHTML="";
  l.forEach(it=>{
    const b=document.createElement("button");
    b.className="chip";b.textContent=it.name;
...
function render(data,src,ts,tz){
  ...
  html+="</table>";
  saida.innerHTML=html;info.textContent=src;
  tempoCache.textContent=ts?`🕒 Última atualização ${tempoDecorrido(ts)}`:"";
  drawChart(data);
}
function drawChart(data){
  const labels=data.map(o=>toLabelDate(o.date)),values=data.map(o=>parseFloat(duracaoH(o.results.day_length)));
  const w=graficoCtx.canvas.width,h=graficoCtx.canvas.height,max=Math.max(...values),min=Math.min(...values);
  const step=w/(values.length-1||1),scale=v=>h-(v-min)*(h/(max-min||1));
  graficoCtx.clearRect(0,0,w,h);
  graficoCtx.beginPath();graficoCtx.strokeStyle=getComputedStyle(document.documentElement).getPropertyValue('--accent');graficoCtx.lineWidth=2;
  values.forEach((v,i)=>{const x=i*step,y=scale(v);i?graficoCtx.lineTo(x,y):graficoCtx.moveTo(x,y);});
  graficoCtx.stroke();
...
/* Inicialização */
function carregar(){
  atualizarStatus();renderRecentChips(loadRecent());
